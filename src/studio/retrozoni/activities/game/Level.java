...
import studio.retrozoni.activities.game.objects.particles.Particle;
...
public class Level implements Receiver {
	...
	private void renderName(Graphics2D g) {
		Font f = FontHandler.font("septem", 10*Engine.SCALE);
		String value = getName();
		int hF = FontHandler.getHeight(value, f);
		int x = getBounds().x;
	...
	private void renderBuilder(Graphics2D g) {
		Font f = FontHandler.font("septem", 8*Engine.SCALE);
		String value = (String)getLevel().get("BUILDER");
		int wF = FontHandler.getWidth(value, f);
		int hF = FontHandler.getHeight(value, f);
