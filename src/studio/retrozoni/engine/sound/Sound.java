...
public class Sound {
	...
	public static void play(Sounds sound) {
		...
		sounds.get(sound.resource()).play((double) Engine.SETTINGS.getInt("VOLUME") / 100d);
	}

	public static void play(Sounds sound, double pan) {
		if(!sounds.containsKey(sound.resource()))
			throw new RuntimeException("sound not exists");
		sounds.get(sound.resource()).play((double) Engine.SETTINGS.getInt("VOLUME") / 100d, pan);
	}
	
	public static void play(Musics music, boolean loop) {
		if(!musics.containsKey(music.resource()))
			throw new RuntimeException("sound not exists");
		musics.get(music.resource()).play(loop, (double) Engine.SETTINGS.getInt("MUSIC") / 100d);
	}

	public static void play(Musics music, boolean loop, double pan) {
		if(!musics.containsKey(music.resource()))
			throw new RuntimeException("sound not exists");
		musics.get(music.resource()).play(loop, (double) Engine.SETTINGS.getInt("MUSIC") / 100d, pan);
	}
